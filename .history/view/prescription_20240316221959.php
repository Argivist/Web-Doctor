        ...
        <div class="site-blocks-table">
          <table class="table table-bordered">
            ...
            <tbody>
              ...
              $cart = getPresc();
              //geting the patient name
              
              $count = 0;
              foreach ($cart as $item) {
                if ($item['approved'] == 1) {
                  $query="SELECT * FROM people WHERE p_id=".$item['custom_id'];
                  $result=mysqli_query($conn,$query);
                  $row=mysqli_fetch_assoc($result);
                  $name=$row['fname']." ".$row['lname'];
                  echo displayPresc($item['medicine_id'], $item['img_url'], $item['medicine_name'], $item['qty'], $item['desc'],$name);
                }
              }
