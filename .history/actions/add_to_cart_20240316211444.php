...
if ($count == 0) {
    ...
    if ($presc['approved'] == 0) {
        header("Location: ../view/shop.php?error='Prescription not approved'&approved='".$presc['approved'];
    } else {
        //check quantity
        if ($presc['qty'] < $qty) {
            ...
            if ($medicine['medicine_qty'] < $qty) {
                ...
                $query = "SELECT * FROM cart WHERE p_id = ? AND med_id = ?";
                $stmt = mysqli_prepare($conn, $query);
                mysqli_stmt_bind_param($stmt, "ii", $pid, $med_id);
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);
                ...
                if ($count > 0) {
                    ...
                    if($row['qty']+$qty>$presc['qty']){
                        header("Location: ../view/shop-single.php?mid=" . $med_id . "&url=" . $url . "&name=" . $med_name . "&price=" . $price . "&desc=" . $desc . "&error='Quantity exceeds prescription'&qty=" . $presc['qty']-$row['qty']);
                    }else{
                        //update cart
                        $query="UPDATE cart SET qty=qty+'".$qty."', total_cost=total_cost+'".$total."' WHERE p_id='".$pid."' AND med_id='".$med_id."'";
