...
if ($presc == null) {
    header("Location: ../view/shop.php?error='No prescription for this medicine'");
} else {
    ...
    if ($presc['approved'] == 0) {
        ...
        if ($presc['qty'] > $qty) {
            ...
            if ($medicine['medicine_qty'] < $qty) {
                header("Location: ../view/shop-single.php?mid=".$med_id."&url=".$url."&name=".$med_name."&price=".$price."&desc=".$desc."&error='Quantity exceeds current stock more will be made available'&qty=" . $presc['qty']);
            } else {
                //Adds to cart
