...
if ($presc == null) {
    ...
    if ($presc['approved'] == 0) {
        ...
        //check quantity
        if ($presc['qty'] > $qty) {
            header("Location: ../view/shop-single.php?shop-single.php?mid=".$med_id."&url=".$url."&name=".$med_name."&price=".$price."&desc=".$desc."&error='Quantity exceeds prescription'&qty=" . $presc['qty']);
        } else {
            ...
            if ($medicine['medicine_qty'] < $qty) {
                header("Location: ../view/shop-single.php?shop-single.php?mid=".$med_id."&url=".$url."&name=".$med_name."&price=".$price."&desc=".$desc."&error='Quantity exceeds current stock more will be made available'&qty=" . $presc['qty']);
            } else {
                //Adds to cart
