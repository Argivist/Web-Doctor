...
<script>
    ...
    function updateItem(id){
      var medinv=fetch("../actions/get_inventory_action.php?id="+id).then(response => response.json()).then(data => {
        return data;});
        document.getElementById("update").innerHTML = `<div id="addItemModal" class="popup" style="display: none;"><div class="popup-item">
                          <form action="../actions/update_inventory_action.php" method="POST">
                                <input type="text" id="productId" name="prodID" value="${id}" hidden><br>
                                <input type="number" id="quantity" name = "quantity"value="`+medinv['qty']+`" placeholder="Quantity"><br>
                                <input type="number" id="price" name = "price" value="`+medinv['price']+`" placeholder="Price"><br>
                                <textarea id="desc" name="desc" value="`+medinv['desc']+`" placeholder="Description"></textarea><br>
