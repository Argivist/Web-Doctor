...
CREATE TABLE `medicine_inventory` (
  ...
  `presc` boolean NOT NULL DEFAULT FALSE,
...
CREATE TABLE `prescription` (
  ...
  foreign key(custom_id) references people(p_id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `cart`(
  ...
  foreign key(p_id) references people(p_id) ON DELETE CASCADE ON UPDATE CASCADE,
  foreign key(med_id) references medicine_inventory(medicine_id) ON DELETE CASCADE ON UPDATE CASCADE);
...
INSERT INTO prescription (custom_id, medicine_id, approved,qty) VALUES
(1, 1, 0,12),
(1, 3, 0,2),
(1, 1, 1,4),
(1, 3, 0,3),
